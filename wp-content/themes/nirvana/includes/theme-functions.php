...
/* FUNCION ORIGINAL, YA QUE PROBABLEMENTE LA VOY A CAMBIAR Y VA A DEJAR DE FUNCIONAR
...
function nirvana_title_and_description() {
	...
	?><div id="header-container"><?php
			...
			$heading_tag = ( ( is_home() || is_front_page() ) && !is_page() ) ? 'h1' : 'div';
			echo '<'.$heading_tag.' id="site-title">';
			echo '<span> <a href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( get_bloginfo( 'name', 'display' ) ) . '" rel="home"><div style="position:absolute;"><img src="http://localhost/proyectos/wrdprss/wp-content/uploads/2017/01/logo-1024x237.png" style="width:30vw; margin: -3% 0 0 3%;"/></div></a> </span>';
			echo '</'.$heading_tag.'>';
	?></div><?php
} // nirvana_title_and_description()
...
function nirvana_set_social_icons($idd) {
	...
	for ($i=1; $i<=9; $i+=2) {
		...
		if ( ${"nirvana_social$j"} ) {
			...
			<a <?php if ($nirvanas['nirvana_social_target'.$i]) {echo ' target="_blank" ';} ?> href="<?php echo esc_url( $cryout_current_social ); ?>"
 ...
 */
 function nirvana_site_info() {
	...
	<?php _e('&reg; 2017 - Incredible Green Box')?> <a target="_blank" href="<?php echo '';?>" title="<?php echo ''.'';?>"><?php echo '' ?></a>
	<a target="_blank" href="<?php echo ''; ?>" title="<?php _e(''); ?>"> <?php printf('' ); ?></a></em>
...
/*function nirvana_site_info() {
    $nirvanas = nirvana_get_theme_options();
    foreach ($nirvanas as $key => $value) { ${"$key"} = $value ; }
	echo '<em style="display:table;margin:0 auto;float:none;text-align:center;padding:7px 0;font-size:13px;">&reg; 2017 - Incredible Green Box</em>';
}
nirvana_site_info();
add_action('cryout_footer_hook','nirvana_site_info',15);*/
